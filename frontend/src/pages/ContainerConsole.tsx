...
export default function ContainerConsole() {
  ...
  const loadInstallation = async () => {
    try {
      const response = await api.get(
        `/api/v1/registry/installations/${connectorId}/status?tenant_id=${tenantId}`
      )
  ...
  const loadLogs = async () => {
    try {
      ...
      const response = await api.get(
        `/api/v1/registry/installations/${connectorId}/logs?tenant_id=${tenantId}&tail=100`
      )
  ...
  const handleRestart = async () => {
    ...
    try {
      await api.post(
        `/api/v1/registry/installations/${connectorId}/restart?tenant_id=${tenantId}`
      )
