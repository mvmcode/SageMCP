...
export default function Installations() {
    ...
    try {
      ...
      for (const tenant of tenantsToLoad) {
        try {
          ...
          const connectorsResponse = await api.get(`/api/v1/admin/tenants/${tenant.slug}/connectors`)
          ...
          const installationPromises = connectors.map(async (connector: any) => {
            try {
              const statusResponse = await api.get(
                `/api/v1/registry/installations/${connector.id}/status?tenant_id=${tenant.id}`
              )
          ...
          const validInstallations = installationsData.filter(i => i !== null)
  ...
  const handleUninstall = async (connectorId: string, tenantId: string) => {
    ...
    try {
      ...
      await api.delete(
        `/api/v1/registry/installations/${connectorId}?tenant_id=${tenantId}`
      )
